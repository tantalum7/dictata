...
# Project imports
from storage.generic_backend import GenericJsonBackend
...
if __name__ == "__main__":
    ...
    backend = FtpJsonBackend(settings=settings)
    ...
    with backend._get_connection() as ftp:

        file = ftp.download("/dictata/bigfile.txt")

        ftp.upload("/dictata/bigfile2.txt", file)

        file2 = ftp.download("/dictata/bigfile.txt")

        ftp.delete("/dictata/bigfile2.txt")
